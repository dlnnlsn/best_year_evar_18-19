...
\[ \iff 0 \leq g +gab -ga -gb = g(a-1)(b-1), \] which is true since $g$, $a$ and $b$ are positive integers. Note that equality occurs if and only if $a$ and $b$ are both equal to $1$, whereupon $m = ga = gb = n$; it is easily checked that when $m = n$ we have equality.
